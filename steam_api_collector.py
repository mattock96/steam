...
from config import STEAM_API_KEY, engine
from sqlalchemy.orm import sessionmaker
from config import Session
from steam_api_converter import (
    converter_json_para_conquista,
    converter_json_para_conquista_por_player,
    converter_json_para_jogo_do_jogador,
    converter_json_para_jogos,
    converter_json_para_perfil,
    converter_json_para_lista_de_amigos,
...
def coletar_dados_e_inserir():
    ...
    for perfil in perfils:
        ...
        if 'response' in response_owned_games.json() and 'games' in response_owned_games.json()['response']:
            ...
            jogos_do_jogador = [converter_json_para_jogo_do_jogador(jogo, perfil.steamid, session) for jogo in
                                data_owned_games]
    ...
    for jogo in total_jogos_do_jogador:
        ...
        if jogo is not None:
            ...
            if response_player_achievements.ok and 'playerstats' in response_player_achievements.json() and 'achievements' in response_player_achievements.json()['playerstats']:
                data_player_achievements = response_player_achievements.json()['playerstats']['achievements']
                conquistas = [converter_json_para_conquista(conquista, jogo.appid, session) for conquista in
                              data_player_achievements]
                conquistas_do_jogador = [converter_json_para_conquista_por_player(conquistad, jogo.steamid, jogo.appid,
                                                                                   session) for conquistad in
                                         data_player_achievements]
                total_conquistas.extend(conquistas)
                total_conquistas_do_jogador.extend(conquistas_do_jogador)

    print('Conquistas coletadas!')
    print('Conquistas dos perfis coletadas!')
